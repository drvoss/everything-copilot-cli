...
|------|-----------|-------|
| Trace data flow through Server Components | `explore` | default |
| Run `prisma migrate`, `npm run build` | `task` | default |
| Add new feature (multi-file) | `general-purpose` | claude-sonnet-4.6 |
| Review PR for Next.js-specific issues | `code-review` | claude-sonnet-4.6 |
| Architecture decisions (App Router vs Pages) | `general-purpose` | claude-opus-4.6 |

### Recommended Skills

Apply these skills from `everything-copilot-cli` to common tasks:

- **`fix-github-issue`** — End-to-end issue resolution with PR creation
- **`commit-workflow`** — Conventional commits for feature/fix/chore changes
- **`pr-multi-perspective-review`** — PM/Dev/QA/Security/DevOps/UX review before merge
- **`security-scan`** — Check Auth.js config, Prisma queries, and Server Action inputs
- **`sprint-workflow`** — Full Think → Plan → Build → Review → Ship for new features
...
# Fix a Prisma query performance issue:
> Use the fix-github-issue skill for issue #42: N+1 query in getUserWithPosts()

# Security audit before launch:
> Apply the security-scan skill focusing on: Auth.js session handling,
> Server Action input validation, and any direct SQL in Prisma queries.
```
